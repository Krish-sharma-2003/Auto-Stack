...
export function ReceiptVoucher() {
  ...
  const [receiptMode, setReceiptMode] = useState<(typeof paymentModes)[number]>('Cash');
  ...
  const voucherNo = `REC-${new Date().getFullYear()}-${String(Math.floor(Math.random() * 1000) + 1).padStart(3, '0')}`;

  const isBankOrCheque = receiptMode === 'Bank' || receiptMode === 'Cheque';
      ...
      >
        ...
        <div className="p-6 border-b border-slate-100 bg-slate-50">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-xl font-bold text-slate-800">Receipt Voucher</h2>
              <p className="text-sm text-slate-500">Record receipt from party</p>
            </div>
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Receipt Mode</label>
              <select
                value={receiptMode}
                onChange={(e) => setReceiptMode(e.target.value as typeof paymentModes[number])}
                className="w-full px-3 py-2 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
              >
          ...
          <div className="p-4 bg-green-50 rounded-lg flex items-center justify-between">
            <span className="font-medium text-slate-700">Receipt Amount</span>
