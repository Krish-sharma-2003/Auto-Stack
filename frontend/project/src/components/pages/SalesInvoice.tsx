...
import { useEffect, useState } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { Plus, Trash2, Printer, Eye, FileText, MessageCircle, UserPlus } from 'lucide-react';
...
export function SalesInvoice() {
  ...
  const loadParties = async () => {
    try {
      ...
      // Silent — party dropdown will just be empty; user can still "+ Add New Party"
    }
  };
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            <div className="md:col-span-2">
              <label className="block text-sm font-medium text-slate-700 mb-1">
                Party Name <span className="text-xs text-slate-400 font-normal">(press F2 to add new)</span>
              </label>
              <select
                value={partyName}
              ...
              >
                ...
                <option value={ADD_NEW_PARTY} className="text-blue-600 font-medium">
                  + Add New Party
                </option>
              </select>
            </div>
